...
public class RestExceptionHandlerInterceptor implements Serializable {
    ...
    public Object handleException(InvocationContext invocationContext) throws Exception {
         ...
         * invocationContext.getTarget().getClass().getName(); final String methodName =
        ...
        try {
            ...
            return retorno;
            /*
             * } catch (AcessoNegadoException an) { RetornoErro<String> acessoNegado = new
             * RetornoErro<String>(); acessoNegado.setMensagem(MENSAGEM.ACESSO_NEGADO);
             * acessoNegado.setDescricaoMensagem(an.getMessage()); return
             * Response.status(Status.UNAUTHORIZED).entity(acessoNegado).build();
             */
            ...
            // LOG.error("Erro NAO tratado pela aplicacao. Operacao [" + methodName + "] em
            // [" + targetClassName + "] - " + ex.getMessage(), ex);
