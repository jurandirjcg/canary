...
import br.com.jgon.canary.ws.rest.util.json.JsonCollectionLinkEntity;
import br.com.jgon.canary.ws.rest.util.json.JsonHALLinkEntity;
import br.com.jgon.canary.ws.rest.util.json.JsonLinkEntity;
...
public class LinkResponseFilter implements ContainerResponseFilter {
    ...
    private void configLinks(UriInfo uriInfo, ContainerResponseContext responseContext) throws ApplicationException {
        ...
        if (linkPaginate != null) {
            paginateLinks
                .addAll(getPaginateLink(responseContext.getEntity(), linkPaginate, uriInfo, path, responseContext.getStringHeaders()));
            ...
            if (isHalLinks) {
                ...
                if (!linkPaginate.collectionLinks().serviceClass().equals(void.class)
                    && StringUtils.isNotBlank(linkPaginate.collectionLinks().serviceMethodName())) {
        ...
        if (isHalLinks && linkResource != null) {
            ...
            if (isCollection) {
                ...
                if (!linkResource.serviceClass().equals(void.class)) {
                    ...
                    LinkResource linkResourceAux = ReflectionUtil.getAnnotation(linkResources.serviceClass(),
                        linkResources.serviceMethodName(), LinkResource.class);
        ...
        if (isHalLinks && linkResources != null) {
            ...
            if (isCollection) {
                ...
                if (!linkResources.serviceClass().equals(void.class)) {
                    Path pathAux = ReflectionUtil.getAnnotation(linkResources.serviceClass(), linkResources.serviceMethodName(),
                        Path.class);
                    linkResAux = new ArrayList<LinkResource>();
                    LinkResources linkResourcesAux = ReflectionUtil.getAnnotation(linkResources.serviceClass(),
                        linkResources.serviceMethodName(), LinkResources.class);
        ...
        if (!linksEntity.isEmpty() || (linkPaginateEntity != null && !linkPaginateEntity.isEmpty())) {
            if (!(responseContext.getEntity() instanceof Collection)) {
                ...
                if (isHalLinks) {
                    responseContext.setEntity(getCollectionEntityLink((Collection<Object>) responseContext.getEntity(), isHalLinks,
                        linkPaginateEntity, uriInfo, attrNotPresentInRequest));
                }
            } else {
                String childName;
                ...
                if (isHalLinks) {
                    json.setEmbedded(getCollectionEntityLink((Collection<Object>) responseContext.getEntity(), isHalLinks,
                        linkPaginateEntity, uriInfo, attrNotPresentInRequest));
                } else {
                ...
                if (json.isHalLink()) {
                    json.setTotalElements(responseContext.getHeaders().getFirst(DominiosRest.X_PAGINATION_TOTAL_ELEMENTS) == null ? null
                        : Long.valueOf(responseContext.getHeaders().getFirst(DominiosRest.X_PAGINATION_TOTAL_ELEMENTS).toString()));
                    json.setElementsPerPage(responseContext.getHeaders().getFirst(DominiosRest.X_PAGINATION_ELEMENTS_PER_PAGE) == null
                        ? null
                        : Integer.valueOf(responseContext.getHeaders().getFirst(DominiosRest.X_PAGINATION_ELEMENTS_PER_PAGE).toString()));
                    json.setCurrentPage(responseContext.getHeaders().getFirst(DominiosRest.X_PAGINATION_CURRENT_PAGE) == null ? null
                        : Integer.valueOf(responseContext.getHeaders().getFirst(DominiosRest.X_PAGINATION_CURRENT_PAGE).toString()));
                    json.setTotalPages(responseContext.getHeaders().getFirst(DominiosRest.X_PAGINATION_TOTAL_PAGE) == null ? null
                        : Integer.valueOf(responseContext.getHeaders().getFirst(DominiosRest.X_PAGINATION_TOTAL_PAGE).toString()));
            ...
            if (responseContext.getEntity() instanceof Collection) {
                ...
                ((Page) responseContext.getEntity())
                    .setElements(getCollectionEntity(((Page) responseContext.getEntity()).getElements(), attrNotPresentInRequest));
            } else {
     ...
     * @throws ApplicationException
     */
    private Collection<Object> getCollectionEntityLink(
        Collection<Object> entityList,
        boolean isHalLinks,
        Map<Path, List<LinkResource>> linkResources,
        UriInfo uriInfo,
        Collection<String> attrNotPresentInRequest) throws ApplicationException {
     ...
     * @throws ApplicationException
     */
    private Collection<Object> getCollectionEntity(Collection<Object> entityList, Collection<String> attrNotPresentInRequest)
        throws ApplicationException {
    ...
    private Link getLink(
        ...
        UriInfo uriInfo,
        List<SimpleEntry<String, Object>> queryParams,
        Object entity,
        Path path,
        boolean forceAbsolutePath) throws ApplicationException {
        return getLink(linkResource.pathParameters(), linkResource.queryParameters(), linkResource.serviceClass(),
            linkResource.serviceMethodName(), linkResource.basePath(), linkResource.includeRequestQueryParams(), linkResource.rel(),
            linkResource.type(), linkResource.title(), uriInfo, queryParams, entity, path, forceAbsolutePath);
    }
     ...
     */
    private Link getLink(
        String[] linkPathParameters,
        String[] linkQueryParameter,
        Class<?> linkServiceClass,
        String linkServiceMethodName,
        LinkResouceBasePath linkBasePath,
        boolean linkIncludeQueryParams,
        String linkRel,
        String linkType,
        String linkTitle,
        UriInfo uriInfo,
...
//            }
        ...
        if (linkIncludeQueryParams) {// || queryParams != null){
            for (String key : uriInfo.getQueryParameters().keySet()) {
                if (uriInfo.getQueryParameters(false).get(key).size() > 1) {
                    ...
                    // uriInfo.getQueryParameters(false).get(key));
                } else {
        ...
        if (linkQueryParameter != null && linkQueryParameter.length > 0) {
            for (int i = 0; i < linkQueryParameter.length; i++) {
                if (StringUtils.isNotBlank(linkQueryParameter[i])) {
                    ...
                    uriBuilder.replaceQueryParam(paramName,
                        valueFromParameter(linkQueryParameter[i].substring(linkQueryParameter[i].indexOf("=") + 1), entity));
                }
            }
        }
     ...
     * @throws ApplicationException
     */
    private List<Link> getPaginateLink(
        Object entity,
        LinkPaginate linkPaginate,
        UriInfo uriInfo,
        Path path,
        MultivaluedMap<String, String> headers) throws ApplicationException {
        ...
        if (!linkPaginate.disablePaginationTemplate()) {
            ...
            queryParamsAux.addAll(queryParams);
            queryParamsAux
                .add(new SimpleEntry<String, Object>(linkPaginate.pageParamName(), linkPaginate.paginationTemplate().pageParamName()));
            if (linkPaginate.paginationTemplate().includeLimitParamTemplate()) {
                queryParamsAux.add(
                    new SimpleEntry<String, Object>(linkPaginate.limitParamName(), linkPaginate.paginationTemplate().limitParamName()));
            }
            paginationLinks
                .add(getLink(linkPaginate.paginationTemplate().pathParameters(), linkPaginate.paginationTemplate().queryParameters(),
                    linkPaginate.paginationTemplate().serviceClass(), linkPaginate.paginationTemplate().serviceMethodName(),
                    linkPaginate.paginationTemplate().basePath(), linkPaginate.paginationTemplate().includeRequestQueryParams(),
                    linkPaginate.paginationTemplate().rel(), linkPaginate.paginationTemplate().type(),
                    linkPaginate.paginationTemplate().title(), uriInfo, queryParamsAux, entity, path, linkPaginate.absolutePath()));
        }
    ...
    /**
     * Retorna o parametro obrigatorio para a cricao do link mas que nao esta na
     * lista de parametros solicitados na requisicao, utilizado para remover o
