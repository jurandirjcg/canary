...
public class WSMapper {
        ...
        if (StringUtils.isNotBlank(campos)) {
            ...
            if (fieldsAjustados != null) {
                ...
                for (String fNome : fldAux) {
                    ...
                    if (fNome.contains(".")) {
                        ...
                        do {
                            ...
                            fldCheck = ReflectionUtil.getAttribute(testClass,
                                fNome.substring(idxStr, idxEnd < 0 ? fNome.length() : idxEnd));
                            if (fldCheck == null) {
                                subObject = false;
                                break;
                            }
                    ...
                    fldCheck = fldCheck != null ? fldCheck
                        : ReflectionUtil.getAttribute(responseClass, fNome.contains(":") ? fNome.substring(0, fNome.indexOf(":")) : fNome);
                    ...
                    if (fldCheck != null && !isPrimitive(fldCheck) && (wsMapperAttribute == null || !wsMapperAttribute.fixed())) {
                        String fldNomeTemp = wsMapperAttribute != null && StringUtils.isNotBlank(wsMapperAttribute.value())
                            ? wsMapperAttribute.value()
                            : fNome;
                        retorno.addAll(verificaCampoObject(fldCheck,
                            !subObject && fNome.contains(".") ? fldNomeTemp + fNome.substring(fNome.length() - 1, fNome.lastIndexOf("."))
                                : fldNomeTemp));
                    } else {
    ...
    private List<String> verificaCampoObject(Field fldCheck, String fNome) {
        ...
        if (!ReflectionUtil.isCollection(fldCheck.getType())) {
            ...
            type = wsMapperAttribute == null || wsMapperAttribute.collectionType().equals(void.class) ? DAOUtil.getCollectionClass(fldCheck)
                : wsMapperAttribute.collectionType();
        }
        ...
        for (Field fldCheckAux : ReflectionUtil.listAttributes(type)) {
            if (isModifierValid(fldCheckAux)) {
                ...
                if (!isPrimitive(fldCheckAux) && (wsMapperAttributeFieldAux == null || !wsMapperAttributeFieldAux.fixed())) {
     ...
     * @return
     */
    private boolean isPrimitive(Field fld) {
        Class<?> klass = fld.getType();
        boolean primitive = isPrimitiveBasic(klass);

        if (!primitive && ReflectionUtil.isCollection(klass)) {
            Class<?> colType = ReflectionUtil.returnParameterType(fld.getGenericType(), 0);
            if (colType != null) {
                primitive = this.isPrimitiveBasic(colType);
            }
        }
        ...
        for (Field fl : fieldClass) {
            if (fl.getName().equals(partField)) {
                ...
                if (/* !isEnum && */multiLevel) {
                    Class<?> attrType = wsMapperAttribute != null && !wsMapperAttribute.collectionType().equals(void.class)
                        ? wsMapperAttribute.collectionType()
                        : fl.getType();
