...
/**
 * Classe responsavel por criar a JPA Criteria e realizar as configuracoes e
 * conversoes para realizar as consultas
 * 
...
class CriteriaManager<T> {
     ...
     */
    public CriteriaManager(EntityManager entityManager, Class<T> entityClass, Class<?> queryClass, Class<?> resultClass,
     ...
     */
    public CriteriaManager(EntityManager entityManager, Class<T> entityClass, CriteriaFilterImpl<T> criteriaFilterUpdate) {
        this.entityManager = entityManager;
...
//		}
     ...
     * @return {@link List}
     */
    private List<Predicate> configAllPredicates(T obj) {
        List<Predicate> predicates = new ArrayList<Predicate>();
        // WHERE
        if (obj != null) {
            predicates.addAll(configPredicates(obj, null, rootEntry));
            ...
            for (String key : criteriaFilter.getListSelection().keySet()) {
                ...
                if (field != null && ReflectionUtil.isCollection(field.getType()) && criteriaFilter.isCollectionSelectionControl()) {
                    ...
                    if (!entityClass.equals(resultClass)) {
                        Field fldResult = ReflectionUtil.getAttribute(resultClass,
                            idxDot >= 0 ? selectionAux.getValue().substring(0, idxDot) : selectionAux.getValue());
                ...
                try {
                    ...
                    logger.error("[configSelections]", MessageSeverity.ERROR, "genericdao-field-not-found", e,
                        assocAux.getValue().getJavaType().getName(), assocAux.getKey());
                    throw e;
                }
    ...
    /*
     * private boolean isModifierValid(Field fld){ boolean valid =
     ...
     */
    /**
     * Configura os JOINS com base nos campos passados como restrições do SELECT ou
     * WHERE
     * 
     * @param from
     * @param key
     ...
     * @return
     */
    private <E> List<Predicate> configPredicates(E obj, String attributeParent, From<?, ?> pathEntry) {
        List<Predicate> predicates = new ArrayList<Predicate>();
        ...
        for (Field field : listFields) {
            ...
            if (checkField(field) && !ReflectionUtil.existAnnotation(field, Transient.class) && !Modifier.isTransient(field.getModifiers())) {
                ...
                attributeName = StringUtils.isBlank(attributeParent) ? field.getName()
                    : attributeParent.concat(".").concat(field.getName());
                ...
                // Where para as listas, somente quando tiver retorno com collection (SELECT
                // EXECUTA PRIMEIRO E SETA A LISTA
    ...
    private <E> void applyPredicate(
        ...
        case IS_NULL:
            ...
            /*
             * }else{ predicates.add(criteriaBuilder.isNull(pathExpression); }
             */
            break;
        case IS_NOT_NULL:
            // if(value == null){
            ...
            /*
             * }else{ predicates.add(criteriaBuilder.isNotNull(pathExpression)); }
             */
            break;
        case LIKE_EXACT:
        ...
        case LIKE_NOT_MATCH_ANYWHERE:
            ...
            break;
        case ILIKE_EXACT:
            predicates.add(criteriaBuilder.like(criteriaBuilder.upper((Expression<String>) pathExpression),
                criteriaBuilder.upper(criteriaBuilder.literal(stringValue))));
            break;
        case ILIKE_NOT_EXACT:
            predicates.add(criteriaBuilder.notLike(criteriaBuilder.upper((Expression<String>) pathExpression),
                criteriaBuilder.upper(criteriaBuilder.literal(stringValue))));
            break;
        case ILIKE_MATCH_END:
            predicates.add(criteriaBuilder.like(criteriaBuilder.upper((Expression<String>) pathExpression),
                criteriaBuilder.upper(criteriaBuilder.literal("%".concat(stringValue)))));
            break;
        case ILIKE_MATCH_START:
            predicates.add(criteriaBuilder.like(criteriaBuilder.upper((Expression<String>) pathExpression),
                criteriaBuilder.upper(criteriaBuilder.literal(stringValue.concat("%")))));
            break;
        case ILIKE_MATCH_ANYWHERE:
            predicates.add(criteriaBuilder.like(criteriaBuilder.upper((Expression<String>) pathExpression),
                criteriaBuilder.upper(criteriaBuilder.literal("%".concat(stringValue).concat("%")))));
            break;
        ...
        case NOT_EQUAL:
            if (value instanceof Collection) {
                ...
                for (Object wValue : listNe) {
                    ...
                    if (isStringType) {
                        predicates.add(criteriaBuilder.notEqual(criteriaBuilder.upper((Expression<String>) pathExpression),
                            criteriaBuilder.upper(criteriaBuilder.literal(wValue.toString()))));
                    } else {
        ...
        case NOT_EQUAL_OTHER_FIELD:
            ...
            if (oFieldNameN.contains(".")) {
                ...
                predicates
                    .add(criteriaBuilder.notEqual(pathExpression, pathAux.get(oFieldNameN.substring(oFieldNameN.lastIndexOf(".") + 1))));
            } else {
        ...
        case LESS_THAN_OTHER_FIELD:
            ...
            if (oFieldNameL.contains(".")) {
                ...
                predicates.add(criteriaBuilder.lessThan(pathEntry.get(attributeName),
                    pathAux.get(oFieldNameL.substring(oFieldNameL.lastIndexOf(".") + 1))));
            } else {
        ...
        case LESS_THAN_OR_EQUAL_TO_OTHER_FIELD:
            ...
            if (oFieldNameLE.contains(".")) {
                ...
                predicates.add(criteriaBuilder.lessThanOrEqualTo(pathEntry.get(attributeName),
                    pathAux.get(oFieldNameLE.substring(oFieldNameLE.lastIndexOf(".") + 1))));
            } else {
        ...
        case GREATER_THAN_OTHER_FIELD:
            ...
            if (oFieldNameG.contains(".")) {
                ...
                predicates.add(criteriaBuilder.greaterThan(pathEntry.get(attributeName),
                    pathAux.get(oFieldNameG.substring(oFieldNameG.lastIndexOf(".") + 1))));
            } else {
        ...
        case GREATER_THAN_OR_EQUAL_TO_OTHER_FIELD:
            ...
            if (oFieldNameGE.contains(".")) {
                ...
                predicates.add(criteriaBuilder.greaterThanOrEqualTo(pathEntry.get(attributeName),
                    pathAux.get(oFieldNameGE.substring(oFieldNameGE.lastIndexOf(".") + 1))));
            } else {
        ...
        default:
            if (value instanceof Collection) {
                ...
                for (Object wValue : list) {
                    ...
                    if (isStringType) {
                        predicates.add(criteriaBuilder.equal(criteriaBuilder.upper((Expression<String>) pathExpression),
                            wValue.toString().toUpperCase()));
                    } else {
    ...
    private <E> List<Predicate> configComplexPredicates(Class<E> obj, String attributeParent, From<?, ?> pathEntry) {
        ...
        for (Field field : listFields) {
            ...
            if (checkField(field) && !ReflectionUtil.existAnnotation(field, Transient.class) && !Modifier.isTransient(field.getModifiers())) {
                ...
                isEntityType = field.getType().isAnnotationPresent(Entity.class) || field.getType().isAnnotationPresent(Embeddable.class);
                isCollectionEntity = ReflectionUtil.existAnnotation(field, OneToMany.class)
                    || ReflectionUtil.existAnnotation(field, ManyToMany.class);
                ...
                if (!listCollectionRelation.isEmpty() && ReflectionUtil.isCollection(field.getType())) {
                    for (String k : criteriaFilter.getWhereRestriction().getRestrictions().keySet()) {
                        ...
                        if (checkKey.equals(field.getName())) {
                            if (listCollectionRelation.containsKey(field.getName())) {
                                listCollectionRelation.get(field.getName()).getWhereRestriction().addAll(k,
                                    criteriaFilter.getWhereRestriction(k));
                            }
                        }
                    }
                }
