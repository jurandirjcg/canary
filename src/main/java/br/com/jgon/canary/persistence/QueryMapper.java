...
/**
 * Realiza o mapeamento dos atributos da Classe de retorno para construção das
 * consultas de seleção e ordenação dos atributos das entidades
 * 
...
abstract class QueryMapper {
    ...
    /**
     * Retorna o campo referenciado da entidade bem como o alias utilizado no objeto
     * de retorno
     * 
     * @param campos
     * @param expression
                ...
                for (String fNome : fldAux) {
                    ...
                    if (fNome.contains(".")) {
                        ...
                        do {
                            ...
                            fldCheck = ReflectionUtil.getAttribute(testClass,
                                fNome.substring(idxStr, idxEnd < 0 ? fNome.length() - idxStr : idxEnd));
                            if (fldCheck == null) {
                                break;
                            }
                    ...
                    if (fldCheck != null && !isPrimitive(fldCheck.getType())) {
                        retorno.addAll(
                            verificaCampoObject(fldCheck, fNome.contains(".") ? fNome.substring(0, fNome.lastIndexOf(".")) : fNome));
                    } else {
            ...
            if (isModifierValid(fldCheckAux)) {
                ...
                if (campoVerificado != null) {
                    if (queryMapperAttribute != null && StringUtils.isNotBlank(queryMapperAttribute.value())) {
                        retorno
                            .add(new SimpleEntry<String, String>(queryMapperAttribute.value().concat(".").concat(campoVerificado.getKey()),
                                queryMapperAttribute.value().concat(".").concat(campoVerificado.getValue())));
                    } else {
                        retorno.add(new SimpleEntry<String, String>(fNome.concat(".").concat(campoVerificado.getKey()),
                            fNome.concat(".").concat(campoVerificado.getValue())));
                    }
                } else {
    ...
    private SimpleEntry<String, String> verificaCampo(Class<?> klass, String fieldName) throws ApplicationRuntimeException {
        ...
        for (Field fl : fieldClass) {
            /*
             * if(fl.isAnnotationPresent(QueryMapperIgnore.class) ||
             * fl.isAnnotationPresent(Transient.class)){ continue; }
             */
            ...
            if (fl.getName().equals(partField)) {
                ...
                if (!isEnum && multiLevel) {
                    ...
                    /*
                     * if(fl.isAnnotationPresent(OneToMany.class) &&
                     * !fl.getAnnotation(OneToMany.class).targetEntity().equals(void.class)){
                     * attrType = fl.getAnnotation(OneToMany.class).targetEntity(); }else
                     * if(fl.isAnnotationPresent(ManyToMany.class) &&
                     * !fl.getAnnotation(ManyToMany.class).targetEntity().equals(void.class)){
                     * attrType = fl.getAnnotation(ManyToMany.class).targetEntity(); }else
                     * if(queryMapperAttribute != null &&
                     * !queryMapperAttribute.collectionTarget().equals(void.class)){ attrType =
                     * queryMapperAttribute.collectionTarget(); }else{ attrType = fl.getType(); }
                     */

                    if (ReflectionUtil.isCollection(attrType)) {
                        ApplicationRuntimeException ae = new ApplicationRuntimeException(MessageSeverity.ERROR,
                            "query-mapper.field-collection-not-definied", klass.getName() + "." + fl.getName());
    ...
    /**
     * Ajusta os campos, caso tenham vindo com o caractere especial "{" Ex:
     * pessoa{id,nome} - retorna pessoa.id,pessoa.nome
     * 
     * @param ini
     * @param exp
     * @return
